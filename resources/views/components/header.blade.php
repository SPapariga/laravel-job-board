<header class="bg-blue-900 text-white p-4" x-data="{ open: false }">
    <div class="container mx-auto flex justify-between items-center">
        ...
        <nav class="hidden md:flex items-center space-x-4">
            <x-nav-link url="/" href="{{ url('/') }}" :active="request()->is('/')"> Home </x-nav-link>
            <x-nav-link url="/jobs" href="{{ url('/jobs') }}" :active="request()->is('jobs')"> All
                Jobs </x-nav-link>
            @auth
                <x-nav-link url="/jobs/saved" href="{{ url('/jobs/saved') }}" :active="request()->is('jobs/saved')"> Saved Jobs </x-nav-link>
                <x-nav-link url="/dashboard" href="{{ url('/dashboard') }}" :active="request()->is('dashboard')" icon="gauge"> Dashboard
                </x-nav-link>
                {{-- <x-logout-button" /> --}}
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ route('dashboard') }}">
                        @if (Auth::user()->avatar)
                            <img src="{{ asset('storage/' . Auth::user()->avatar) }}" alt="{{ Auth::user()->avatar }}"
                                class="w-10 h-10 rounded-full" />
                        @else
                            <img src="{{ asset('storage/avatars/default-avatar.png') }}" alt="{{ Auth::user()->avatar }}"
                                class="w-10 h-10 rounded-full" />
                        @endif
            ...
            @else
                <x-nav-link url="/login" href="{{ url('/login') }}" :active="request()->is('login')"> Login </x-nav-link>
                <x-nav-link url="/register" href="{{ url('/register') }}" :active="request()->is('register')"> Register </x-nav-link>
            @endauth
